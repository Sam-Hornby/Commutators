...
BOOST_AUTO_TEST_CASE(empty) {
  ...
  exp = perform_multiplication_substitutions<Fock1DInfo>(
      exp, all_states_orthognal<Fock1DInfo>);
  std::stringstream ss;
  exp.print(ss);
...
BOOST_AUTO_TEST_CASE(no_subsi) {
  ...
  exp = perform_multiplication_substitutions<GenericInfo>(exp,
                                                          no_subs<GenericInfo>);
...
static bool
test_sub(typename vector_type<Operator<GenericInfo>>::iterator start,
...
BOOST_AUTO_TEST_CASE(equal_states) {
  auto exp = hermition_conjugate<Fock1DInfo>(vacuum_state<Fock1DInfo>()) *
...
BOOST_AUTO_TEST_CASE(unequal_states) {
  auto exp = Operator<Fock1DInfo>(ordering_value(0),
                                  Fock1DInfo(1, Type::HC_STATE_VECTOR)) *
             vacuum_state<Fock1DInfo>();
  std::stringstream ss;
  exp.print(ss);
...
BOOST_AUTO_TEST_CASE(op_and_state) {
  Operator<Fock1DInfo> A(ordering_value(-1),
                         Fock1DInfo(0, Type::ANIHILATION_OPERATOR));

  auto exp1 = A * vacuum_state<Fock1DInfo>();
  exp1 = perform_multiplication_substitutions<Fock1DInfo>(
      exp1, all_states_orthognal<Fock1DInfo>);
  std::stringstream ss;
  exp1.print(ss);
  BOOST_CHECK_EQUAL(ss.str(), "a_0 * |0>\n");

  auto exp2 = hermition_conjugate(vacuum_state<Fock1DInfo>()) * A;
  exp2 = perform_multiplication_substitutions<Fock1DInfo>(
      exp2, all_states_orthognal<Fock1DInfo>);
...
BOOST_AUTO_TEST_CASE(destroy_vacuum) {
  ...
  exp = perform_multiplication_substitutions<Fock1DInfo>(
      exp, anihilate_vacuum<Fock1DInfo>);
  exp = simplify_numbers<Fock1DInfo>(exp);
  std::stringstream ss;
  exp.print(ss);
  BOOST_CHECK_EQUAL(ss.str(), "\n");

  exp = hermition_conjugate(vacuum_state<Fock1DInfo>()) * creation_op(0) *
        vacuum_state<Fock1DInfo>();
  exp = perform_multiplication_substitutions<Fock1DInfo>(
      exp, anihilate_vacuum<Fock1DInfo>);
  exp = simplify_numbers<Fock1DInfo>(exp);
...
BOOST_AUTO_TEST_CASE(destroy_fermions) {
  auto exp = anihilation_op(1) * anihilation_op(1) * creation_op(1) *
             vacuum_state<Fock1DInfo>();
  exp = perform_multiplication_substitutions<Fock1DInfo>(
      exp, fermion_dual_occupation<Fock1DInfo>);
  exp = simplify_numbers<Fock1DInfo>(exp);
  std::stringstream ss;
  exp.print(ss);
  BOOST_CHECK_EQUAL(ss.str(), "\n");

  exp = anihilation_op(1) * anihilation_op(2) * creation_op(1) *
        vacuum_state<Fock1DInfo>();
  exp = perform_multiplication_substitutions<Fock1DInfo>(
      exp, fermion_dual_occupation<Fock1DInfo>);
  exp = simplify_numbers<Fock1DInfo>(exp);
