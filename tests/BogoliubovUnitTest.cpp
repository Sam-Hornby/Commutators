...
BOOST_AUTO_TEST_CASE(SuccessfulyTransform_1) {
  ...
  BOOST_TEST_MESSAGE("Op types " + 
                      std::to_string(static_cast<unsigned>(b_test_trans.get_op_type())));
  const auto input = (E * ((c1_h * c1) + (c2_h * c2)))
                   + (Y * ((c1_h * c2_h) + (c2 * c1)));

  const auto f_out =
      bogoliubov_transform<GenericInfo>(input, f_test_trans);

  const auto b_out =
      bogoliubov_transform<GenericInfo>(input, b_test_trans);
...
BOOST_AUTO_TEST_CASE(SimpleInvalidTransforms) {
  ...
  const auto out1_d = bogoliubov_transform<GenericInfo>(input1, OperatorType::FERMION);
  ...
  const auto out2_d = bogoliubov_transform<GenericInfo>(input2, OperatorType::FERMION);
  ...
  const auto out3_d = bogoliubov_transform<GenericInfo>(input3, OperatorType::FERMION);
...
BOOST_AUTO_TEST_CASE(InvalidTransform_1) {
  ...
  const auto out_d = bogoliubov_transform<GenericInfo>(input, OperatorType::FERMION);
...
BOOST_AUTO_TEST_CASE(NonQuadraticTransform) {
  ...
  const auto out4_d = bogoliubov_transform<GenericInfo>(input4, OperatorType::BOSON);
  BOOST_CHECK_EQUAL(out4.name(), fmt::format("(c3) + {}", transformed_b_string));
  BOOST_CHECK_EQUAL(out4_d.name(), fmt::format("(c3) + {}", transformed_b_string));
  ...
  const auto out5_d = bogoliubov_transform<GenericInfo>(input5, OperatorType::BOSON);
  BOOST_CHECK_EQUAL(out5.name(), fmt::format("(Y) + {}", transformed_b_string));
  BOOST_CHECK_EQUAL(out5_d.name(), fmt::format("(Y) + {}", transformed_b_string));
