...
BOOST_AUTO_TEST_CASE(onetoone) {
  ...
  auto new_exp =
      transform_expression<GenericInfo, GenericInfo>(e * other, test_transform);
  BOOST_CHECK_EQUAL(new_exp.print(true), "E * other\n");
  new_exp =
      transform_expression<GenericInfo, GenericInfo>(other * e, test_transform);
...
BOOST_AUTO_TEST_CASE(multiple_terms) {
  ...
  auto new_exp =
      transform_expression<GenericInfo, GenericInfo>(e * a, test_transform);
  BOOST_CHECK_EQUAL(new_exp.print(true), "(E * A) + (E * B)\n");
  new_exp =
      transform_expression<GenericInfo, GenericInfo>(a * c, test_transform);
  BOOST_CHECK_EQUAL(new_exp.print(true),
                    "(A * C) + (B * C) + (A * D) + (B * D)\n");
  new_exp =
      transform_expression<GenericInfo, GenericInfo>(f * a, test_transform);
  BOOST_CHECK_EQUAL(new_exp.print(true), "(F * G * A) + (F * G * B)\n");
  new_exp =
      transform_expression<GenericInfo, GenericInfo>(a * f, test_transform);
  BOOST_CHECK_EQUAL(new_exp.print(true), "(A * F * G) + (B * F * G)\n");
  new_exp =
      transform_expression<GenericInfo, GenericInfo>(a + f, test_transform);
  BOOST_CHECK_EQUAL(new_exp.print(true), "(A) + (B) + (F * G)\n");

  new_exp = transform_expression<GenericInfo, GenericInfo>(h * a + (h * f),
                                                           test_transform);
