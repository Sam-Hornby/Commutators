...
static void random_sort_test(const unsigned seed, const std::size_t n,
                             const int max, const int min, const bool inter) {
  ...
  Operator<GenericInfo> A(ordering_value(reference[0]),
                          GenericInfo(std::to_string(reference[0])));
  Operator<GenericInfo> B(ordering_value(reference[1]),
                          GenericInfo(std::to_string(reference[1])));
  Expression<GenericInfo> exp = A * B;
  for (unsigned i = 2; i < reference.size(); ++i) {
    exp =
        exp * Operator<GenericInfo>(ordering_value(reference[i]),
                                    GenericInfo(std::to_string(reference[i])));
  }
  ...
  const Expression<GenericInfo> new_exp = [&]() {
    if (inter) {
      return interleaved_evaluate<GenericInfo>(
          exp, commute_all<GenericInfo>,
  ...
  for (unsigned i = 0; i < reference.size(); ++i) {
    ...
    BOOST_CHECK_EQUAL(std::to_string(new_exp.expression[0][i].order.value),
                      new_exp.expression[0][i].name());
  }
}
...
static void random_multiply_and_addition(const unsigned seed,
  ...
  for (const auto &vec : reference) {
    Expression<GenericInfo> mul_term =
        Operator<GenericInfo>(ordering_value(vec[0]),
                              GenericInfo(std::to_string(vec[0]))) *
        Operator<GenericInfo>(ordering_value(vec[1]),
                              GenericInfo(std::to_string(vec[1])));
    for (unsigned j = 2; j < vec.size(); ++j) {
      mul_term =
          mul_term * Operator<GenericInfo>(ordering_value(vec[j]),
                                           GenericInfo(std::to_string(vec[j])));
    }
    exp = exp + mul_term;
  }
  ...
  for (unsigned i = 0; i < reference.size(); ++i) {
    for (unsigned j = 0; j < reference[i].size(); ++j) {
      ...
      BOOST_CHECK_EQUAL(std::to_string(new_exp.expression[i][j].order.value),
                        new_exp.expression[i][j].name());
    }
  }
}
...
BOOST_AUTO_TEST_CASE(numberss) {
  auto exp = Operator<GenericInfo>(ordering_value(0), GenericInfo("A")) *
             named_number<GenericInfo>('z') * Operator<GenericInfo>(3);
...
BOOST_AUTO_TEST_CASE(inter_empty) {
  Expression<Fock1DInfo> exp;
  auto new_exp = interleaved_evaluate<Fock1DInfo>(
      exp, commute_all<Fock1DInfo>,
...
BOOST_AUTO_TEST_CASE(inter_one) {
  ...
  exp.expression.resize(3);
  exp.expression[0].push_back(A);
  auto new_exp = interleaved_evaluate<GenericInfo>(
      exp, commute_all<GenericInfo>,
      [](typename vector_type<Operator<GenericInfo>>::iterator,
         vector_type<Operator<GenericInfo>> &) { return false; });
  std::stringstream ss;
...
BOOST_AUTO_TEST_CASE(inter_number) {
  ...
  exp = interleaved_evaluate<GenericInfo>(
      exp, commute_all<GenericInfo>,
      [](typename vector_type<Operator<GenericInfo>>::iterator,
         vector_type<Operator<GenericInfo>> &) { return false; });
  std::stringstream ss;
