...
struct DefaultBogTransformFunctions : public BogTransformFunctionsBase<Info> {
  ...
  unsigned get_order(const Operator<Info> &) const override { return 1U; }
  ...
  unsigned get_index(const Info &op) const override { std::abort(); }
  OperatorPairs<Info> transform_ops(OperatorPairs<Info> input) const override {
    input.creation_ops[0].info() =
        default_transform(input.creation_ops[0].info());
    input.creation_ops[1].info() =
        default_transform(input.creation_ops[1].info());
    input.anihilation_ops[0].info() =
        default_transform(input.anihilation_ops[0].info());
    input.anihilation_ops[1].info() =
        default_transform(input.anihilation_ops[1].info());
    return input;
  }
  DefaultBogTransformFunctions(OperatorType op_type)
      : BogTransformFunctionsBase<Info>(op_type) {
...
struct GenericInfoBogTransform
    : public DefaultBogTransformFunctions<GenericInfo> {
  ...
  OperatorPairs<GenericInfo>
  transform_ops(OperatorPairs<GenericInfo> input) const override {
    // dont copy this as not very general, just using for tests
    spdlog::trace("Transform opertor pairs begin");
    spdlog::trace("{} {} {} {}", input.creation_ops[0].name(),
                  input.creation_ops[1].name(), input.anihilation_ops[0].name(),
                  input.anihilation_ops[1].name());
  ...
  GenericInfoBogTransform()
      : DefaultBogTransformFunctions<GenericInfo>(op_type_test) {
