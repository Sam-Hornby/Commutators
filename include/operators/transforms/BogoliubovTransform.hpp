...
// A bogoliubov transform only works on second order terms, we can skip all
// other terms
...
template <class InfoA>
static unsigned
get_order(const Operator<InfoA> &A,
...
template <class InfoA>
static unsigned
get_order(const vector_type<Operator<InfoA>> &term,
          const BogTransformFunctionsBase<InfoA> &transformFunctions) {
  return std::accumulate(term.begin(), term.end(), 0U,
                         [&](unsigned acc, const Operator<InfoA> &A) {
...
get_grouping(const vector_type<Operator<InfoA>> &term,
             const BogTransformFunctionsBase<InfoA> &transformFunctions) {
...
create_final_expression(BogExpressionPair<InfoA> &pair,
  ...
  for (auto &expr : groups.transformable_terms) {
    result = result +
             expr; // TODO don't use this + loop as O(n**2), just insert back
  }
  return simplify_numbers(result);
}

template <class InfoA>
Expression<InfoA> error_commutator(const Operator<InfoA> &A,
                                   const Operator<InfoA> &B) {
  return commute_numbers<InfoA>(
      A, B,
      [](const Operator<InfoA> &,
         const Operator<InfoA> &) -> Expression<InfoA> {
...
// TODO pile
//  - make sure not just that only right bumber of creation / anihilation but
//  also they
...
template <class InfoA>
static TermType
get_term_type(const vector_type<Operator<InfoA>> &term,
              const BogTransformFunctionsBase<InfoA> &transformFunctions) {
...
static Expression<InfoA> convert_op_to_expression(Operator<InfoA> op) {
  ...
  result.expression.emplace_back(
      vector_type<Operator<InfoA>>(1, std::move(op)));
  return result;
}

template <class InfoA>
static Expression<InfoA>
get_constant(const vector_type<Operator<InfoA>> &term) {
  ...
  for (unsigned i = 1U; i < term.size(); ++i) {
    if (!term[i].is_number()) {
      break; // we sorted the terms before this step so all numbers should be a
             // front
    }
...
template <class InfoA>
const Expression<InfoA> BogConstants<InfoA>::zero_expression = {
    {{Operator<InfoA>(0)}}};

template <class InfoA>
static BogConstants<InfoA>
get_bog_constants(const Expression<InfoA> &input,
                  const BogTransformFunctionsBase<InfoA> &transformFunctions) {
  ...
  for (const auto &term : input.expression) {
    ...
    if (term_type == TermType::DIAGONOL_TERM and
        result.E_constant == result.zero_expression) {
      result.E_constant = std::move(constant);
    } else if (term_type == TermType::CROSS_TERM and
               result.Y_constant == result.zero_expression) {
...
template <class InfoA>
static OperatorPairs<InfoA>
get_bog_operators(const Expression<InfoA> &exp,
                  const BogTransformFunctionsBase<InfoA> &transformFunctions) {
  ...
  for (const auto &term : exp.expression) {
    for (const auto &op : term) {
      if (std::all_of(already_found.begin(), already_found.end(),
                      [](bool a) { return a; })) {
        break;
      }
      if (op.is_number()) {
  ...
  assert(std::all_of(already_found.begin(), already_found.end(),
                     [](bool a) { return a; }));
  return result;
}
...
template <class InfoA>
static bool
can_transform(const Expression<InfoA> &input,
              const BogTransformFunctionsBase<InfoA> &transformFunctions) {
  ...
  for (const auto &term : input.expression) {
    ...
    spdlog::trace("Term type = {}",
                  term_type_names[static_cast<unsigned>(type)]);
    ...
    auto &old_constant =
        type == TermType::DIAGONOL_TERM ? E_constant : Y_constant;
    if (old_constant) {
  ...
  return expression_matches<InfoA>(
      input, get_bog_operators<InfoA>(input, transformFunctions),
...
// e (b_0! b_0 + b_1! b_0) + K
// Constant e and K are determined by E, Y and whether the transforms are on
// fermions or bosons
template <class InfoA>
static SeperatedTerms<InfoA>
can_transform(SeperatedTerms<InfoA> input,
              const BogTransformFunctionsBase<InfoA> &transformFunctions) {
...
template <class InfoA>
TransformedBogConstants<InfoA>
get_transformed_constants(BogConstants<InfoA> &input, const bool fermions) {
  ...
  if (fermions) {
    auto e_constant = SquareRootExpr::create(AddExpr::create(
        SquareExpr::create(E_constant), SquareExpr::create(Y_constant)));
    result.e_constant = {{{Operator<InfoA>(e_constant)}}};
    K_constant = SubExpr::create(E_constant, e_constant);
  } else {
    auto e_constant = SquareRootExpr::create(SubExpr::create(
        SquareExpr::create(E_constant), SquareExpr::create(Y_constant)));
    result.e_constant = {{{Operator<InfoA>(e_constant)}}};
    K_constant = SubExpr::create(e_constant, E_constant);
  }
  ...
  assert(K_constant);
  if (input.constants.expression ==
      BogConstants<InfoA>::zero_expression.expression) {
    result.K_constant = {{{Operator<InfoA>(*K_constant)}}};
  } else {
    result.K_constant = {
        {{Operator<InfoA>(AddExpr::create(constants, *K_constant))}}};
  }
  return result;
}
...
static Expression<InfoA> make_transformation(
  ...
  auto transformed_ops =
      get_transformed_operators<InfoA>(input, transformFunctions);
  return create_new_expression<InfoA>(std::move(transformed_ops),
                                      std::move(transformed_constants));
}

template <class InfoA>
static std::vector<Expression<InfoA>> make_transformation(
...
static void log_groups(const SeperatedTerms<InfoA> &groups) {
  if (!groups.non_transformable_terms.expression.empty()) {
    spdlog::warn("Some terms cannot be transformed {}",
                 groups.non_transformable_terms.print(false));
  }
...
template <class Info>
OpPairType<Info> find_op_pair(const vector_type<Operator<Info>> &term) {
...
template <class Info>
void maybeInsertInfosAndGroup(
    std::map<Info, std::optional<GroupAndIndex>> &result,
    const OpPairType<Info> &op_pair) {

  const std::array all_terms{op_pair.first.info(), op_pair.second.info(),
  ...
  const std::array its{result.find(all_terms[0]), result.find(all_terms[1]),
                       result.find(all_terms[2]), result.find(all_terms[3])};
  if (std::all_of(its.begin(), its.end(),
                  [&](const auto &it) { return it == result.end(); })) {
  ...
  if (std::any_of(its.begin(), its.end(), [&](const auto &it) {
        return it == result.end() or (not static_cast<bool>(it->second)) or
...
template <class Info>
std::map<Info, GroupAndIndex>
create_group_map(const Expression<Info> &input,
                 const BogTransformFunctionsBase<Info> &transformFunctions) {
  auto expressions =
      seperate_second_order_terms<Info>(input, transformFunctions);
  expressions.second_order_terms = simplify_numbers(sort<Info>(
      numbers_first(expressions.second_order_terms), error_commutator<Info>));
  ...
  for (const auto &term : expressions.second_order_terms.expression) {
    ...
    // when find cross term insert both ops and the hermitian conj of both into
    // map with same group. If already in map dunno
  ...
  for (auto &item : provisionalResult) {
    if (item.second) {
      // maybe instead of not inserting should just seperate out all these so
      // they have a unique group each. means won't fail if can't transform but
      // won't try to transform
...
bogoliubov_transform(const Expression<InfoA> &input,
  ...
  auto expressions =
      seperate_second_order_terms<InfoA>(input, transformFunctions); // done
  spdlog::debug("Non second order terms: {}",
                expressions.other_terms.print(false));
  spdlog::debug("Second order terms: {}",
                expressions.second_order_terms.print(false));
  spdlog::info("Pair up second order terms");
  auto groups = group_terms<InfoA>(expressions.second_order_terms,
                                   transformFunctions); // done
  log_groups(groups);
  spdlog::info("Check transformable");
  groups = can_transform<InfoA>(std::move(groups),
                                transformFunctions); // get term type todo pile
  log_groups(groups);
  spdlog::info("Try to make substitutions");
  // todo: get new constants from old constants; implement create new term
  // function taking transformed constants
  groups.transformable_terms = make_transformation<InfoA>(
      std::move(groups.transformable_terms), transformFunctions);

  auto result = create_final_expression(expressions, groups, input);
  if (recurse) {
    return bogoliubov_transform(result, SwapIndices<InfoA>(transformFunctions),
                                false);
  }
  return result;
}
...
template <class InfoA>
Expression<InfoA> bogoliubov_transform(const Expression<InfoA> &input,
  ...
  FindGroupsTransformFunctions<InfoA> complete_transform_functions(
      transformFunctions, create_group_map<InfoA>(input, transformFunctions));
  spdlog::info("Create group map with {} entries",
               transformFunctions.groupMap.size());
